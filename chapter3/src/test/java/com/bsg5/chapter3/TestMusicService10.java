...
public class TestMusicService10 extends AbstractTestNGSpringContextTests {
    ...
    private void runMethod(Object config, Consumer<MusicService> method) {
        ...
        if (config instanceof String) {
            ...
            throw new RuntimeException(String.format("Invalid config %s\n", config));
        }

        MusicService musicService = context.getBean(MusicService.class);
        method.accept(musicService);
    }
    ...
    @Test(dataProvider = "configurations")
    public void testService(Object config) {
        runMethod(config, tests::testService);
    }
}
